...
bool parse_cli(int argc, char** argv, CliConfig* config, std::string* error) {
    ...
    if (config->inject == "timeout") {
        config->timeout_at = config->timeout_at < 0 ? 5 : config->timeout_at;
        config->allowed_timeouts = config->recover ? 1 : 0;
    } else if (config->inject == "bytesused_zero") {
        config->bytesused_zero_at = config->bytesused_zero_at < 0 ? 3 : config->bytesused_zero_at;
    } else if (config->inject == "source_change") {
        config->source_change_at = config->source_change_at < 0 ? 4 : config->source_change_at;
    ...
    if (config->min_decoded_frames < 0 || config->allowed_timeouts < 0) {
        if (error) *error = "min-decoded-frames/allowed-timeouts must not be negative";
        return false;
    }
    return true;
}
...
void print_usage(const char* argv0) {
    std::cout << "Usage: " << argv0 << " [options]\n"
              ...
              << "  --inject=none|timeout|bytesused_zero|source_change|source_change_no_reconfigure\n"
              << "  --frames=12 --output-depth=3 --capture-depth=4\n"
              << "  --timeout-at=5 --source-change-at=4 --bytesused-zero-at=3\n"
              << "  --min-decoded-frames=6 --allowed-timeouts=0\n"
              << "  --output-fourcc=H264 --capture-fourcc=NV12 --width=1280 --height=720\n"
              << "  --require-device --no-recover --quiet\n";
}
