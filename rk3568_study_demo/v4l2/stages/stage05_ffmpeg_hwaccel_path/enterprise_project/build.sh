...
if [[ -z "${FF_LIBS}" ]]; then
  echo "[enterprise build] missing ffmpeg dev libs" >&2
  echo "sudo apt-get install -y ffmpeg libavformat-dev libavcodec-dev libavutil-dev pkg-config" >&2
  echo "or: FFMPEG_PREFIX=/opt/rockchip/ffmpeg-rockchip ./build.sh" >&2
  exit 2
fi

mkdir -p "${BIN_DIR}"
OUT="${BIN_DIR}/09_enterprise_ffmpeg_hwaccel_service"
...
"${CXX}" ${CXXFLAGS} ${FF_CFLAGS} -I"${INC_DIR}" \
  ...
  "${SRC_DIR}/06_hwaccel_pipeline_service.cpp" \
  "${SRC_DIR}/07_enterprise_pipeline_main.cpp" \
  -o "${OUT}" ${FF_LIBS}

echo "[enterprise build] output=${OUT}"
